...
class QyweixinTask extends \Phalcon\CLI\Task
{
    ...
    public function getchatdataAction(array $params)
    {
        ...
        foreach ($maxseqList as $maxseqItem) {
            try {
                // 获取agentInfo信息
                $agentInfo = $modelAgent->getInfoByAppid($maxseqItem['provider_appid'], $maxseqItem['authorizer_appid'], $maxseqItem['agentid'], false);
                if (empty($agentInfo)) {
                    continue;
                }
                $snList = $modelSn->getListByCorpid($maxseqItem['agentid'], $maxseqItem['authorizer_appid'], $maxseqItem['provider_appid']);
                if (empty($snList)) {
                    continue;
                }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_department_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
        ...
        try {
            ...
            if (!empty($departmentList)) {
                foreach ($departmentList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_department_user_simple_list:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $deptid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($departmentUserList)) {
                ...
                foreach ($departmentUserList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_user_list_from_department_user:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $userid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
                    ...
                    try {
                        $userInfo = $modelUser->getInfoByUserId($userid, $authorizer_appid, $provider_appid, $externaluser_agent_agentid);
                        if (empty($userInfo)) {
                            $data = array();
                            $data['userid'] = $userid;
        ...
        try {
            ...
            if (!empty($userList)) {
                foreach ($userList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_user_info:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $userid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_tag_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($tagList)) {
                foreach ($tagList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_tag_party_user:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $tagid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_follow_user_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($ecFolowUserList)) {
                foreach ($ecFolowUserList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_external_user_list:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $externaluser_follow_user;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($ecUserList)) {
                foreach ($ecUserList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_external_user_info:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $external_userid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($ecFolowUserList)) {
                foreach ($ecFolowUserList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_group_chat_list:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $externaluser_follow_user;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($groupChatList)) {
                foreach ($groupChatList as $info) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_group_chat_info:' . $externaluser_agent_agentid . ':' . $authorizer_appid . ':' . $provider_appid . ":" . $chat_id;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                ...
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_moment_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                ...
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_groupmsg_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
    ...
    public function getgroupmsgtasklistAction(array $params)
    {
        ...
        try {
            foreach ($msgidList as $info) {
                ...
                // 如果缓存中已经存在那么就不做处理
                $cacheKey = 'get_groupmsg_task_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid . ':' . $msgid;
                $userFromCache = $cache->get($cacheKey);
                if (!empty($userFromCache)) {
                    continue;
                }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'get_corp_tag_list:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'getuserbehaviordata:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
        ...
        try {
            ...
            if (!empty($agentList)) {
                foreach ($agentList as $agentItem) {
                    ...
                    // 如果缓存中已经存在那么就不做处理
                    $cacheKey = 'getgroupchatstatistic:' . $agentid . ':' . $authorizer_appid . ':' . $provider_appid;
                    $userFromCache = $cache->get($cacheKey);
                    if (!empty($userFromCache)) {
                        continue;
                    }
