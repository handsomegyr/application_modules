                    ...
                    if (!empty($taskInfo['userids'])) {
                        ...
                        foreach ($chunkArr as $useridChunk) {
                            if ($taskInfo['notification_method'] == \App\Qyweixin\Models\Notification\Task::NOTIFY_BY_EXTERNALCONTACT_ADD_MSG_TEMPLATE && $msgTemplateInfo['chat_type'] == 'single') { // 4:发送企业消息到群聊会话
                                $modelTaskLog->log($taskInfo['provider_appid'], $taskInfo['authorizer_appid'], $taskProcessItem['_id'], $taskInfo['_id'], $taskInfo['name'], $taskInfo['notification_method'],  $taskInfo['externalcontact_msg_template_chat_type'], $taskInfo['agent_msg_id'], $taskInfo['appchat_msg_id'], $taskInfo['externalcontact_msg_template_id'], $taskInfo['linkedcorp_msg_id'], $taskInfo['changemsginfo_callback'], 0, '按userids列表发送消息', \App\Common\Utils\Helper::myJsonEncode($useridChunk), "", \App\Qyweixin\Models\Notification\TaskProcess::PUSHING, $now);
                            } else {
                        ...
                        foreach ($chunkArr as $useridChunk) {
                            if ($taskInfo['notification_method'] == \App\Qyweixin\Models\Notification\Task::NOTIFY_BY_EXTERNALCONTACT_ADD_MSG_TEMPLATE && $msgTemplateInfo['chat_type'] == 'single') { // 4:发送企业消息到群聊会话
                                $modelTaskLog->log($taskInfo['provider_appid'], $taskInfo['authorizer_appid'], $taskProcessItem['_id'], $taskInfo['_id'], $taskInfo['name'], $taskInfo['notification_method'],  $taskInfo['externalcontact_msg_template_chat_type'], $taskInfo['agent_msg_id'], $taskInfo['appchat_msg_id'], $taskInfo['externalcontact_msg_template_id'], $taskInfo['linkedcorp_msg_id'], $taskInfo['changemsginfo_callback'], 0, '按上传文件发送消息', \App\Common\Utils\Helper::myJsonEncode($useridChunk), "", \App\Qyweixin\Models\Notification\TaskProcess::PUSHING, $now);
                            } else {
                                foreach ($useridChunk as $userid) {
