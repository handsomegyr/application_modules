...
class ComponentController extends ControllerBase
{
    ...
    private $modelQyweixinKeywordToCustomMsg;
    ...
    /**
     * @var \App\Qyweixin\Models\Qrcode\EventLog
     */
    ...
    public function initialize()
    {
        ...
        $this->modelQyweixinKeywordToCustomMsg = new \App\Qyweixin\Models\Keyword\KeywordToCustomMsg();
    ...
    /**
     * 登录授权的发起
     * 当全网发布之后，需要做一个页面，上面加一个link，地址就是以下的地址，做法如http://weshopdemo.umaman.com/provider/login.html
     * 方式一：授权注册页面扫码授权
     *
     * 授权页网址为：
     *
    ...
    {
        // http://wxcrm.eintone.com/provider/login.html
        // http://wxcrmdemo.jdytoy.com/qyweixin/api/provider/login?appid=wxca8519f703c07d32&redirect=https%3A%2F%2Fwww.baidu.com%2F
        // http://wxcrm.eintone.com/qyweixin/api/provider/login?appid=wxca8519f703c07d32&redirect=https%3A%2F%2Fwww.baidu.com%2F
    ...
    /**
     * 登录授权的发起
     * 当全网发布之后，需要做一个页面，上面加一个link，地址就是以下的地址，做法如http://weshopdemo.umaman.com/provider/bind.html
     * 方式二：点击移动端链接快速授权
     * 第三方服务商方可以生成授权链接，将链接通过移动端直接发给授权管理员，管理员确认后即授权成功。
     * 授权链接为：
     *
    ...
    {
        // http://wxcrm.eintone.com/provider/bind.html
        // http://wxcrmdemo.jdytoy.com/qyweixin/api/provider/bind?appid=wxca8519f703c07d32&redirect=https%3A%2F%2Fwww.baidu.com%2F
        // http://wxcrm.eintone.com/qyweixin/api/provider/bind?appid=wxca8519f703c07d32&redirect=https%3A%2F%2Fwww.baidu.com%2F
        $_SESSION['oauth_start_time'] = microtime(true);
        try {
            // 初始化
            $this->doInitializeLogic();
    ...
    /**
     * 步骤3：用户确认并同意登录授权给第三方服务商方
     *
     * 用户进入第三方服务商授权页后，需要确认并同意将自己的公众号或小程序授权给第三方服务商方，完成授权流程。
     *
     ...
     *
     * 在得到授权码后，第三方服务商方可以使用授权码换取授权公众号或小程序的接口调用凭据（authorizer_access_token，也简称为令牌），再通过该接口调用凭据，按照公众号开发者文档或小程序开发文档的说明，去调用公众号或小程序相关API。
     * （能调用哪些API，取决于用户将哪些权限集授权给了第三方服务商方，也取决于公众号或小程序自身拥有哪些接口权限），使用JS SDK等能力。具体请见【公众号第三方服务商的接口说明】
     *
     * 下面对各API和机制进行介绍（特别注意，所有API调用需要验证调用者IP地址。只有在第三方服务商申请时填写的白名单IP地址列表内的IP地址，才能合法调用，其他一律拒绝）：
     */
    public function logincallbackAction()
    {
        // http://wxcrm.eintone.com/qyweixin/api/provider/logincallback?appid=wxca8519f703c07d32&auth_code=queryauthcode@@@YGWj2XtzxxwQQfpIW6VGOJvDALlLfFwSy9anTBQs0sEMXsXZMrTRVPbhXpmqh265vpKz_YIgj2bJ-WJR2oGABw&expires_in=3600
        try {
            // 初始化
            $this->doInitializeLogic();
            ...
            $this->modelQyweixinAuthorizer->createAndUpdateAuthorizer($this->appConfig['appid'], $authorizationInfo['authorizer_appid'], $authorizationInfo['authorizer_access_token'], $authorizationInfo['authorizer_refresh_token'], $authorizationInfo['expires_in'], $authorizationInfo['func_info'], $memo);
            ...
            $this->modelQyweixinProviderLoginBindTracking->record($this->appid, $this->trackingKey, $_SESSION['oauth_start_time'], microtime(true), $authorizationInfo['authorizer_appid']);
    ...
    /**
     * 授权事件接收URL
     * 在公众号第三方服务商创建审核通过后，微信服务器会向其“授权事件接收URL”每隔10分钟定时推送component_verify_ticket。第三方服务商方在收到ticket推送后也需进行解密（详细请见【消息加解密接入指引】），接收到后必须直接返回字符串success。
     */
    public function authorizecallbackAction()
    {
        // http://wxcrm.eintone.com/qyweixin/api/provider/authorizecallback?appid=wxca8519f703c07d32
        try {
            /**
             * ==================================================================================
             * ====================================以下逻辑请勿修改===============================
            ...
            if ($InfoType == 'component_verify_ticket') { // 推送component_verify_ticket协议
                ...
                // 获取第三方服务商component_access_token
                $componentToken = $this->objWeixinComponent->apiComponentToken($componentVerifyTicket);

                // 更新component_access_token
                $this->modelQyweixinProvider->updateAccessToken($this->appConfig['_id'], $componentToken['component_access_token'], $componentToken['expires_in'], $componentVerifyTicket);

                // 消息解密
            } elseif ($InfoType == 'unauthorized') { // 取消授权通知
                /**
                 * <xml>
                 * <AppId>第三方服务商appid</AppId>
     ...
     *
     * * 公众号消息与事件接收URL
     * http://wxcrm.eintone.com/qyweixin/api/provider/$APPID$/callback?appid=wxca8519f703c07d32
     * 处理微信的回调数据
     *
     * 1、在接收已授权公众号消息和事件的URL中，增加2个参数（此前已有2个参数，为时间戳 timestamp，随机数nonce），分别是encrypt_type（加密类型，为aes）和msg_signature（消息体签名，用于验证消息体的正确性）
     * 2、postdata中的XML体，将使用第三方服务商申请时的接收消息的加密symmetric_key（也称为EncodingAESKey）来进行加密。
     *
     *
     * @return boolean
     */
    ...
    {
        // http://wxcrm.eintone.com/qyweixin/api/provider/$APPID$/callback?appid=wxca8519f703c07d32&authorizer_appid=xxx
        // http://wxcrm.eintone.com/qyweixin/api/provider/xxxxxxx/callback?appid=wxca8519f703c07d32&authorizer_appid=xxx
        try {
            /**
             * ==================================================================================
             * ====================================以下逻辑请勿修改===============================
            ...
            $provider_appid = $this->appConfig['appid'];
            ...
            if (!empty($this->authorizerConfig['access_token'])) {
                $this->modelQyweixinUser->setWeixinInstance($this->objWeixin);
                $this->modelQyweixinUser->updateUserInfoByAction($FromUserName, $this->authorizerConfig['appid'], $this->appConfig['appid']);
            }
            // 设定来源和目标用户的openid
    ...
    {
        // http://wxcrm.eintone.com/qyweixin/api/provider/authorizer-token?appid=wxca8519f703c07d32&authorizer_appid=xxx
        try {
            // 初始化
            $this->doInitializeLogic();
            ...
            $authorizerInfo = $this->modelQyweixinAuthorizer->getInfoByAppId($this->appid, $authorizer_appid);

            if (empty($authorizerInfo)) {
                throw new \Exception("provider_appid:{$this->appid}和authorizer_appid:{$authorizer_appid}所对应的记录不存在");
            }
            ...
            // 更新accesstoken
            $ret = $this->modelQyweixinAuthorizer->updateAccessToken($authorizerInfo['_id'], $authorizerTokenInfo['authorizer_access_token'], $authorizerTokenInfo['authorizer_refresh_token'], $authorizerTokenInfo['expires_in'], null);
    ...
    {
        // http://wxcrm.eintone.com/qyweixin/api/provider/get-component-access-token?appid=wxca8519f703c07d32
        try {
            // 初始化
            $this->doInitializeLogic();
    ...
    public function testGetUserAction()
    {
        ...
        try {
            ...
            $this->modelQyweixinUser->setWeixinInstance($this->objWeixin);
            $ret = $this->modelQyweixinUser->updateUserInfoByAction($FromUserName, $this->authorizerConfig['appid'], $this->appConfig['appid']);
    ...
    public function testKeywordAction()
    {
        ...
        try {
            ...
            // 为回复的Model装载weixin对象
            $this->modelQyweixinReplyMsg->setWeixinInstance($this->objWeixin);
    ...
    protected function verifyComponent($datas)
    {
        ...
        if (in_array($ToUserName, array(
            ...
            // 1、模拟粉丝触发专用测试公众号的事件，并推送事件消息到专用测试公众号，第三方服务商方开发者需要提取推送XML信息中的event值，并在5秒内立即返回按照下述要求组装的文本消息给粉丝。
            // 1）微信推送给第三方服务商方： 事件XML内容（与普通公众号接收到的信息是一样的）
            ...
            // 2、模拟粉丝发送文本消息给专用测试公众号，第三方服务商方需根据文本消息的内容进行相应的响应：
            // 1）微信模推送给第三方服务商方：文本消息，其中Content字段的内容固定为：TESTCOMPONENT_MSG_TYPE_TEXT
            // 2）第三方服务商方立马回应文本消息并最终触达粉丝：Content必须固定为：TESTCOMPONENT_MSG_TYPE_TEXT_callback
            elseif ($content == 'TESTCOMPONENT_MSG_TYPE_TEXT') {
                $msg = "{$content}_callback";
            }

            // 3、模拟粉丝发送文本消息给专用测试公众号，第三方服务商方需在5秒内返回空串表明暂时不回复，然后再立即使用客服消息接口发送消息回复粉丝
            // 1）微信模推送给第三方服务商方：文本消息，其中Content字段的内容固定为： QUERY_AUTH_CODE:$query_auth_code$（query_auth_code会在专用测试公众号自动授权给第三方服务商方时，由微信后台推送给开发者）
            // 2）第三方服务商方拿到$query_auth_code$的值后，通过接口文档页中的“使用授权码换取公众号的授权信息”API，将$query_auth_code$的值赋值给API所需的参数authorization_code。然后，调用发送客服消息api回复文本消息给粉丝，其中文本消息的content字段设为：$query_auth_code$_from_api（其中$query_auth_code$需要替换成推送过来的query_auth_code）
    ...
    protected function doInitializeLogic()
    {
        ...
        // 创建service
        $this->weixinopenService = new \App\Qyweixin\Services\WeixinService($this->authorizer_appid, $this->appid);
        $this->appConfig = $this->weixinopenService->getAppConfig4Provider();
        if (empty($this->appConfig)) {
            throw new \Exception("appid:{$this->appid}所对应的记录不存在");
        }
        $this->objWeixinComponent = $this->weixinopenService->getWeixinProvider();
        ...
        if (!empty($this->authorizer_appid)) {
            ...
            if (empty($this->authorizerConfig)) {
                throw new \Exception("provider_appid:{$this->appid}和authorizer_appid:{$this->authorizer_appid}所对应的记录不存在");
            }
    ...
    protected function handleRequestAndGetResponseByEvent(array $datas)
    {
        ...
        if ($Event == 'subscribe') { // 关注事件
            ...
            if (!empty($Ticket) && !empty($EventKey)) { // 扫描带参数二维码事件 1. 用户未关注时，进行关注后的事件推送
                ...
                $this->modelQyweixinQrcodeEventLog->record($this->authorizer_appid, $this->appid, $AgentID, $scene_id, $FromUserName, $ToUserName, $CreateTime, $MsgType, $Event, $EventKey, $Ticket);
                $this->modelQyweixinQrcode->incSubscribeEventNum($this->authorizer_appid, $this->appid, $AgentID, $scene_id, 1);
            ...
            $this->modelQyweixinQrcodeEventLog->record($this->authorizer_appid, $this->appid, $AgentID, $EventKey, $FromUserName, $ToUserName, $CreateTime, $MsgType, $Event, $EventKey, $Ticket);
            $this->modelQyweixinQrcode->incScanEventNum($this->authorizer_appid, $this->appid, $AgentID, $EventKey, 1);
    ...
    protected function answer($FromUserName, $ToUserName, $content, $authorizer_appid, $provider_appid, $agentid)
    {
        ...
        if (empty($match)) {
            ...
            if (empty($match)) {
                $match = $this->modelQyweixinKeyword->matchKeyWord('默认回复', "", $provider_appid, 0, false);
            }
        }
        $match['reply_msg_ids'] = $this->modelQyweixinKeywordToReplyMsg->getReplyMsgIdsByKeywordId($match['_id']);
        $match['custom_msg_ids'] = $this->modelQyweixinKeywordToCustomMsg->getCustomMsgIdsByKeywordId($match['_id']);
        $match['template_msg_ids'] = $this->modelQyweixinKeywordToTemplateMsg->getTemplateMsgIdsByKeywordId($match['_id']);
