...
class HongbaoController extends ControllerBase
{
    ...
    {
        try {
            // http://www.myapplicationmodule.com/hongbao/index/index?FromUserName=ol6BkwxV-PJVlfRUd5nhUJ_EGM20&istest=1
    ...
    {
        // http://www.myapplicationmodule.com/hongbao/index/withdraw?FromUserName=xxxx&money=100
        try {
            $this->view->disable();
    ...
    {
        try {
            // http://www.myapplicationmodule.com/hongbao/index/hongbaoauthorize?callbackUrl=xxx
            ...
            if (!empty($FromUserName)) {
                ...
                if ($this->validateOpenid4Guotai($FromUserName, $timestamp, $secretKey, $signkey)) {
                    ...
                    $key = \App\Common\Utils\Helper::myCacheKey(__CLASS__, $FromUserName);
                    $objLock = new \iLock($key);
                    if ($objLock->lock()) {
                        $this->refreshPage(5);
                    }
    ...
    {
        // http://www.myapplicationmodule.com/hongbao/index/sendredpack
        try {
            $this->view->disable();
