...
class PayController extends ControllerBase
{
    ...
    {
        try {
            // http://www.applicationmodule.com/order/pay/create?pay_sn=567e1fd8887c22034a8b45a3&integral=0&predeposit=1&payway=weixin&pay_pwd=xxxx
            // 订单支付单号
    ...
    {
        try {
            // http://www.applicationmodule.com/order/pay/recharge?predeposit=10&payway=weixin&pay_pwd=xxx
            // 预付款
            $predeposit = intval($this->get('predeposit', '0'));
            // 支付方式
            $payment_code = $payway = ($this->get('payway', ''));
    ...
    {
        try {
            // http://www.applicationmodule.com/order/pay/finish?out_trade_no=577791f1887c22a4328b457a
            // 订单支付单号
    ...
    {
        try {
            // http://www.applicationmodule.com/order/pay/getpayresult?id=56640956887c22014a8b457c
            // 支付id
            $id = trim($this->get('id', ''));
