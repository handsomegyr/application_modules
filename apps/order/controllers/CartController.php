...
class CartController extends ControllerBase
{
    ...
    {
        try {
            // http://webcms.didv.cn/order/cart/add?goods_id=56372bd07f50eab004000443&quantity=1
    ...
    {
        try {
            // http://webcms.didv.cn/order/cart/updatenum?goods_id=56372bd07f50eab004000443&quantity=1
    ...
    {
        try {
            // http://webcms.didv.cn/order/cart/clear?goods_ids=56372bd07f50eab004000443,xxx,xxx
    ...
    {
        // http://webcms.didv.cn/order/cart/cartnum
        try {
            $info = array(
                'num' => 0,
                'money' => 0.00
    ...
    public function cartlabelAction()
    {
        ...
        // http://webcms.didv.cn/order/cart/cartlabel
        try {
            $buyer_id = empty($_SESSION['member_id']) ? '' : $_SESSION['member_id'];
            // 获取购物车
            $cart = $this->serviceCart->getCartByBuyerId($buyer_id);
    ...
    {
        try {
            // http://webcms.didv.cn/order/cart/checkout?goods_ids=xx,xxx,xxx&order_from=1&order_message=xxxx&pay_name=online
            // 购买商品信息
