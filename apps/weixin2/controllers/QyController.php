...
class QyController extends ControllerBase
{
    ...
    /**
     * @var \App\Weixin2\Models\Keyword\KeywordToReplyMsg
     */
    private $modelWeixinopenKeywordToReplyMsg;

    /**
     * @var \App\Weixin2\Models\Keyword\KeywordToAgentMsg
     */
    private $modelWeixinopenKeywordToAgentMsg;

    /**
     * @var \App\Weixin2\Models\Keyword\KeywordToTemplateMsg
     */
    ...
    // lock key
    private $lock_key_prefix = 'weixinopen_qy_';

    private $trackingKey = "公众号授权给第三方平台流程";

    /** @var  \Weixin\Component */
    private $objQyWeixinComponent;
    ...
    public function initialize()
    {
        ...
        $this->modelWeixinopenKeywordToAgentMsg = new \App\Weixin2\Models\Keyword\KeywordToAgentMsg();
    ...
    {
        // http://wxcrmdemo.jdytoy.com/weixinopen/api/qy/callback?component_appid=qy_ww975831ac45517bff&authorizer_appid=ww975831ac45517bff
        // http://wxcrm.eintone.com/weixinopen/api/qy/callback?component_appid=qy_ww975831ac45517bff&authorizer_appid=ww975831ac45517bff
        try {
            /**
             * ==================================================================================
             * ====================================以下逻辑请勿修改===============================
            ...
            $AESInfo['component_appid'] = isset($_GET['component_appid']) ? trim(($_GET['component_appid'])) : '';
            ...
            if ($this->request->isGet()) {
                ...
                if (!empty($MsgId)) {
                    $uniqueKey = $MsgId . "-" . $this->componentConfig['appid'] . "-" . $this->authorizerConfig['appid'] . "-" . $AgentID;
                } else {
                    $uniqueKey = $FromUserName . "-" . $CreateTime . "-" . $this->componentConfig['appid'] . "-" . $this->authorizerConfig['appid'] . "-" . $AgentID;
                }
                ...
                if (!empty($AgentID)) {
                    // 创建service
                    $this->qyweixinService = new \App\Weixin2\Services\QyService($this->authorizer_appid, $this->component_appid, $AgentID);
                    $this->objQyWeixin = $this->qyweixinService->getQyWeixinObject();
                }
                // // 获取微信用户的个人信息
                // if (!empty($this->authorizerConfig['access_token'])) {
                //     $this->modelWeixinopenUser->setWeixinInstance($this->objQyWeixin);
                //     $this->modelWeixinopenUser->updateUserInfoByAction($FromUserName, $this->authorizerConfig['appid'], $this->componentConfig['appid']);
                // }
                // 设定来源和目标用户的openid
    ...
    public function testDecryptMsgAction()
    {
        ...
        try {
            ...
            $verifyToken = isset($this->componentConfig['verify_token']) ? $this->componentConfig['verify_token'] : '';
            $encodingAesKey = isset($this->componentConfig['EncodingAESKey']) ? $this->componentConfig['EncodingAESKey'] : '';
            $AppId = $this->componentConfig['appid'];
    ...
    public function testGetUserAction()
    {
        ...
        try {
            ...
            $this->modelWeixinopenUser->setWeixinInstance($this->objQyWeixin);
            $ret = $this->modelWeixinopenUser->updateUserInfoByAction($FromUserName, $this->authorizerConfig['appid'], $this->componentConfig['appid']);
    ...
    public function testKeywordAction()
    {
        ...
        try {
            ...
            // 为回复的Model装载weixin对象
            $this->modelWeixinopenReplyMsg->setWeixinInstance($this->objQyWeixin);

            $response = $this->answer("FromUserName", "ToUserName", $keyword, $this->authorizerConfig['appid'], $this->componentConfig['appid'], $agentid);
    ...
    protected function doInitializeLogic()
    {
        ...
        // 创建service
        $this->qyweixinService = new \App\Weixin2\Services\QyService($this->authorizer_appid, $this->component_appid, 0);
        $this->componentConfig = $this->qyweixinService->getAppConfig4Component();
        if (empty($this->componentConfig)) {
            throw new \Exception("component_appid:{$this->component_appid}所对应的记录不存在");
        }
        $this->objQyWeixinComponent = $this->qyweixinService->getWeixinComponent();
    ...
    protected function answer($FromUserName, $ToUserName, $content, $authorizer_appid, $component_appid, $agentid)
    {
        ...
        $match['agent_msg_ids'] = $this->modelWeixinopenKeywordToAgentMsg->getAgentMsgIdsByKeywordId($match['id']);
