...
CREATE TABLE `ipoints_category` (
  ...
  PRIMARY KEY (`_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='积分-分类';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分-日志';
...
CREATE TABLE `ipoints_rule` (
  ...
  `__REMOVED__` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否删除',
  PRIMARY KEY (`_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='积分-规则';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='积分-用户';
