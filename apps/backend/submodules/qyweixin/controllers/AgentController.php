...
class AgentController extends \App\Backend\Controllers\FormController
{
    ...
    {
        // http://www.myapplicationmodule.com/admin/qyweixin/agent/getaccesstoken?id=xxx
        try {
    ...
    {
        // http://www.myapplicationmodule.com/admin/qyweixin/agent/getagentlist?id=xxx
        try {
            $id = trim($this->request->get('id'));
            if (empty($id)) {
                return $this->makeJsonError("记录ID未指定");
    ...
    {
        // http://www.myapplicationmodule.com/admin/qyweixin/agent/getagentinfo?id=xxx
        try {
            $id = trim($this->request->get('id'));
            if (empty($id)) {
                return $this->makeJsonError("记录ID未指定");
    ...
    protected function getSchemas2($schemas)
    {
        ...
        $agenttypeItems = array(
            0 => '企业内部应用',
            1 => '自定义应用'
        );
        $schemas['agent_type'] = array(
            'name' => '应用类型',
            'data' => array(
                'type' => 'integer',
                'length' => 11,
        ...
        $schemas['agent_jsapi_ticket'] = array(
            'name' => '应用JSAPI Ticket',
            'data' => array(
                'type' => 'string',
                'length' => 1024,
        ...
        $schemas['agent_jsapi_ticket_expire'] = array(
            'name' => '应用JSAPI Ticket过期时间',
            'data' => array(
                'type' => 'datetime',
                'length' => 19,
                'defaultValue' => getCurrentTime()
