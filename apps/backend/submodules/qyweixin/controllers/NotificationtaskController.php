    ...
    public function addtask4Action()
    {
        ...
        try {
            ...
            if ($this->request->isGet()) {
                ...
                $fields['externalcontact_msg_template_id'] = array(
                    'name' => '企业群发消息ID',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                $name = trim($this->request->get('name')); // 任务名
                $externalcontact_msg_template_id = trim($this->request->get('externalcontact_msg_template_id')); // 企业群发消息ID
                $changemsginfo_callback = trim($this->request->get('changemsginfo_callback'));
                $userids = trim($this->request->get('userids')); // userid列表
                $userids_sql = trim($this->request->get('userids_sql')); // 获取userid的sql文
                $scheduled_push_time = trim($this->request->get('scheduled_push_time')); // 预定推送时间
                ...
                if (empty($externalcontact_msg_template_id)) {
                    return $this->makeJsonError("企业群发消息ID未指定");
                }
                $isValid = $this->modelTask->checkChangemsginfoCallbackIsValid($changemsginfo_callback);
                if (empty($isValid)) {
                    return $this->makeJsonError("消息内容修改回调函数不合法");
    ...
    public function modifytask4Action()
    {
        ...
        try {
            ...
            if ($this->request->isGet()) {
                ...
                $name = trim($this->request->get('name')); // 任务名
                $externalcontact_msg_template_id = intval($this->request->get('externalcontact_msg_template_id')); // 企业群发消息ID
                $changemsginfo_callback = trim($this->request->get('changemsginfo_callback'));
                $userids = trim($this->request->get('userids')); // userid列表
                $userids_sql = trim($this->request->get('userids_sql')); // 获取userid的sql文
                $scheduled_push_time = trim($this->request->get('scheduled_push_time')); // 预定推送时间
