    ...
    protected function getFormTools2($tools)
    {
        $tools['getmedia'] = array(
            ...
            'is_show' => function ($row) {
                if (!empty($row) && !empty($row['authorizer_appid']) && !empty($row['provider_appid']) && !empty($row['media_id'])) {
                    return true;
                } else {
                    return false;
                }
        ...
        $tools['uploadmedia'] = array(
            ...
            'action' => 'uploadmedia',
            'is_show' => function ($row) {
                if (!empty($row) && !empty($row['authorizer_appid']) && !empty($row['provider_appid']) && !empty($row['type'])) {
