 ...
 * @name 用户增减数据
 */
class DatacubeusersummaryController extends \App\Backend\Controllers\FormController
{
    ...
    protected function addCustomActions4Grid(\Encore\Admin\Grid $grid)
    {
        //$grid->exporter(new \App\Admin\Controllers\Order\Exporters\Order\Goods());
        $grid->tools(function (\Encore\Admin\Grid\Tools $tools) {
            $tools->append(new \App\Admin\Controllers\Weixinopen\Actions\DataCube\UserSummary\SyncUserSummary());
        });
    }

    protected function getHeaderTools2($tools)
    {
        $tools['syncusersummary'] = array(
            'title' => '获取用户增减数据',
            'action' => 'syncusersummary',
            'is_show' => true,
            'is_export' => false,
    ...
    public function syncusersummaryAction()
    {
        ...
        try {
            ...
            if ($this->request->isGet()) {
                ...
                // 起始日期
                $fields['usersummary_start_ref_date'] = array(
                    'name' => '起始日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                // 结束日期
                $fields['usersummary_end_ref_date'] = array(
                    'name' => '结束日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                $component_appid = trim($this->request->get('usersummary_component_appid'));
                $authorizer_appid = trim($this->request->get('usersummary_authorizer_appid'));
                $start_ref_date = trim($this->request->get('usersummary_start_ref_date'));
                $end_ref_date = trim($this->request->get('usersummary_end_ref_date'));
                ...
                $res = $weixinopenService->syncUserSummary($start_ref_date, $end_ref_date);
    ...
    {
        $fields = array();
        $fields['usersummary_component_appid'] = array(
            'name' => '第三方平台应用ID',
            'validation' => array(
                'required' => true
            ),
        ...
        $fields['usersummary_authorizer_appid'] = array(
            'name' => '授权方应用ID',
            'validation' => array(
                'required' => true
            ),
