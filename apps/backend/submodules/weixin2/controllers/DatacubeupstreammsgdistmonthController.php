...
use App\Backend\Submodules\Weixin2\Models\DataCube\UpstreamMsgDistMonth;
use App\Backend\Submodules\Weixin2\Models\Authorize\Authorizer;
use App\Backend\Submodules\Weixin2\Models\Component\Component;

/**
 * @title({name="消息发送分布月数据"})
 *
 * @name 消息发送分布月数据
 */
class DatacubeupstreammsgdistmonthController extends \App\Backend\Controllers\FormController
{
    private $modelUpstreamMsgDistMonth;
    private $modelAuthorizer;
    private $modelComponent;
    public function initialize()
    {
        $this->modelUpstreamMsgDistMonth = new UpstreamMsgDistMonth();
    ...
    {
        $tools['syncUpstreammsgdistmonth'] = array(
            'title' => '获取消息发送分布月数据',
            'action' => 'syncUpstreammsgdistmonth',
            'is_show' => true,
            'is_export' => false,
     ...
     * @name 获取消息发送分布月数据
     */
    public function syncUpstreammsgdistmonthAction()
    {
        // http://www.applicationmodule.com/admin/weixin2/datacubeupstreammsgdistmonth/syncUpstreammsgdistmonth?id=xxx
        try {
            $this->view->disable();
            ...
            if ($this->request->isGet()) {
                ...
                // 起始日期
                $fields['upstreammsgdistmonth_start_ref_date'] = array(
                    'name' => '起始日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                // 结束日期
                $fields['upstreammsgdistmonth_end_ref_date'] = array(
                    'name' => '结束日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                $component_appid = trim($this->request->get('upstreammsgdistmonth_component_appid'));
                $authorizer_appid = trim($this->request->get('upstreammsgdistmonth_authorizer_appid'));
                $start_ref_date = trim($this->request->get('upstreammsgdistmonth_start_ref_date'));
                $end_ref_date = trim($this->request->get('upstreammsgdistmonth_end_ref_date'));
    ...
    {
        $fields = array();
        $fields['upstreammsgdistmonth_component_appid'] = array(
            'name' => '第三方平台应用ID',
            'validation' => array(
                'required' => true
            ),
        ...
        $fields['upstreammsgdistmonth_authorizer_appid'] = array(
            'name' => '授权方应用ID',
            'validation' => array(
                'required' => true
            ),
    ...
    {
        return $this->modelUpstreamMsgDistMonth;
    }
}
