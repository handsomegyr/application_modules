...
use App\Backend\Submodules\Weixin2\Models\DataCube\ArticleSummary;
 ...
 * @name 图文群发每日数据
 */
class DatacubearticlesummaryController extends \App\Backend\Controllers\FormController
{
    private $modelArticleSummary;
    private $modelAuthorizer;
    private $modelComponent;
    public function initialize()
    {
        $this->modelArticleSummary = new ArticleSummary();
    ...
    {
        $tools['syncarticlesummary'] = array(
            'title' => '获取图文群发每日数据',
            'action' => 'syncarticlesummary',
            'is_show' => true,
            'is_export' => false,
    ...
    /**
     * @title({name="获取图文群发每日数据"})
     *
     * @name 获取图文群发每日数据
     */
    public function syncarticlesummaryAction()
    {
        // http://www.applicationmodule.com/admin/weixin2/datacubearticlesummary/syncarticlesummary?id=xxx
        try {
            $this->view->disable();
            ...
            if ($this->request->isGet()) {
                ...
                // 起始日期
                $fields['articlesummary_start_ref_date'] = array(
                    'name' => '起始日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                // 结束日期
                $fields['articlesummary_end_ref_date'] = array(
                    'name' => '结束日期',
                    'validation' => array(
                        'required' => true
                    ),
                ...
                $component_appid = trim($this->request->get('articlesummary_component_appid'));
                $authorizer_appid = trim($this->request->get('articlesummary_authorizer_appid'));
                $start_ref_date = trim($this->request->get('articlesummary_start_ref_date'));
                $end_ref_date = trim($this->request->get('articlesummary_end_ref_date'));
                ...
                $res = $weixinopenService->syncArticleSummary($start_ref_date, $end_ref_date);
    ...
    {
        $fields = array();
        $fields['articlesummary_component_appid'] = array(
            'name' => '第三方平台应用ID',
            'validation' => array(
                'required' => true
            ),
        ...
        $fields['articlesummary_authorizer_appid'] = array(
            'name' => '授权方应用ID',
            'validation' => array(
                'required' => true
            ),
    ...
    {
        return $this->modelArticleSummary;
    }
}
