...
( function($) {
	$.facebox = function(data, klass) {
		...
		/*
		 * if (data.ajax) fillFaceboxFromAjax(data.ajax, klass) else if
		 * (data.image) fillFaceboxFromImage(data.image, klass) else if
		 * (data.div) fillFaceboxFromHref(data.div, klass) else if
		 * ($.isFunction(data)) data.call($) else $.facebox.reveal(data, klass)
		 */
	}
	...
	function fillFaceboxFromHref(href, klass) {
		...
		if (href.match(/#/)) {
			...
			if (target == '#')
				return $.facebox.reveal($(target).html(), klass)
	...
	function showOverlay() {
		...
		$('#facebox_overlay').css("width", iWidth + "px");
		$('#facebox_overlay').css("height", bg_h + "px");
		return false
	}
